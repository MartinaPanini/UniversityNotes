...
def NoiseExtract(Im,qmf,sigma,L):
    ...
    nr = (np.ceil((M+minpad)/m)*m).astype(int)
    nc = (np.ceil((N+minpad)/m)*m).astype(int)  # dimensions of the padded image (always pad 8 pixels or more)
...
def WienerInDFT(ImNoise,sigma):
    ...
    NoiseClean = NoiseClean.astype(np.float32)
