...
% Define the true position of the target (out ground truth, which is not
...
function estimated_position = trilateration(anchors, distances)
    ...
    for i = 1:n-1
        ...
        b(i) = - distances(i+1)^2  + distances(i)^2 + anchors(i+1, 1)^2 - anchors(i, 1)^2 ... 
        + anchors(i+1, 2)^2 - anchors(i, 2)^2;
    end

    estimated_position = (H' * H)^-1 * H' * b;
    % S\p
end
